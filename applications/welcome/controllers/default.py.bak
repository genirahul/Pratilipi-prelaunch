...
#subscription confirmation mail send to the users.
def email(to,name,mail_type,url):
    from gluon.tools import Mail
    mail = Mail()
    mail.settings.server = 'gae'
    mail.settings.sender = 'contact.pratilipi@gmail.com'
    mail.settings.login = 'contact.pratilipi@gmail:1Pratilipi'
...
def index():
    ...
    form =subscribe_form()
    if db(db.subscribers.email==request.args(0)).select()==1:
        session.flash="You have already subscribed."
...
def invite_friends():
    url = "https://genial-charter-608.appspot.com/%s" %URL('subscribe',args=[request.args(0)])
